...
class ConstructTest < AlleleImage::TestCase
  context "a Conditional Allele" do
    setup do
      file       = TEST_ROOT + "/../misc/project_47462_conditional_allele.gbk"
  ...
  context "a Conditional Vector" do
    setup do
      file       = TEST_ROOT + "/../misc/project_47462_conditional_vector.gbk"
      @construct = AlleleImage::Parser.new( file ).construct
    end
  ...
  context "A mirKO vector" do
    should "have cassette features" do
      @filename     = "#{TEST_ROOT}/../misc/mirko/20797.gbk"
      @image_file   = @filename.gsub(/gbk$/, 'gif')

      @allele_image = AlleleImage::Image.new(@filename)
      assert_not_nil @allele_image.construct, "AlleleImage::Construct is nil"
      assert_not_empty @allele_image.construct.cassette_features, "No cassette features"
